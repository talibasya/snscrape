...
class _TwitterAPIScraper(snscrape.base.Scraper):
	...
	def _ensure_guest_token(self, url = None):
		...
		self._async_client.cookies.set('gt', self._guestTokenManager.token, domain = '.twitter.com', path = '/')
	...
	def _unset_guest_token(self):
		...
		del self._async_client.cookies['gt']
	...
	async def _get_api_data(self, endpoint, apiType, params):
		self._ensure_guest_token()
		...
		r = await self._async_get(endpoint, params = params, headers = self._apiHeaders, responseOkCallback = self._check_api_response)
		try:
			obj = r.json()
		except json.JSONDecodeError as e:
	...
	async def _iter_api_data(self, endpoint, apiType, params, paginationParams = None, cursor = None, direction = _ScrollDirection.BOTTOM):
		...
		while True:
			...
			obj = await self._get_api_data(endpoint, apiType, reqParams)
			yield obj
	...
	async def _v2_timeline_instructions_to_tweets(self, obj, includeConversationThreads = False):
		# No data format test, just a hard and loud crash if anything's wrong :-)
		for instruction in obj['timeline']['instructions']:
			...
			for entry in entries:
				if entry['entryId'].startswith('sq-I-t-') or entry['entryId'].startswith('tweet-'):
					...
					async for v in self._v2_instruction_tweet_entry_to_tweet(entry['entryId'], entry['content'], obj):
						yield v
					...
					for item in entry['content']['timelineModule']['items']:
						if item['entryId'].startswith('tweet-'):
							...
							async for v in self._v2_instruction_tweet_entry_to_tweet(item['entryId'], item, obj):
								yield v

	async def _v2_instruction_tweet_entry_to_tweet(self, entryId, entry, obj):
...
class TwitterSearchScraper(_TwitterAPIScraper):
	...
	async def get_items(self):
		...
		async for obj in self._iter_api_data('https://api.twitter.com/2/search/adaptive.json', _TwitterAPIType.V2, params, paginationParams, cursor = self._cursor):
			async for v in self._v2_timeline_instructions_to_tweets(obj):
				yield v
...
class TwitterUserScraper(TwitterSearchScraper):
	...
	async def _get_entity(self):
		...
		obj = await self._get_api_data(endpoint, _TwitterAPIType.GRAPHQL, params = variables)
